    ...
    @if($search_position || !(!$search_position && $positions->isEmpty()))
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <form action="{{ url("") }}/admin/vagas" method="GET" class="" id="form-search">
                    ...
                    <table class="table" id="" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            @foreach ($positions as $position)
                                    <tr>
                                        <td >
                                            <form action="{{ url("") }}/admin/vagas/{{$position->id}}" method="post" id="form-ordenation">
                                                @method('PUT')
                                                @csrf
                                        ...
                                        <td ><a href="{{ url("") }}/admin/vagas/{{ $position->id }}/admin/grupo/vagas/{{ $position->group_id }}">{{ $position->group_id }}</a></td>
                                        <td ><a href="{{ url("") }}/admin/vagas/{{ $position->id }}/edit">{{ $position->title }}</a></td>
                                        
                                        <td class="list-actions">
                                            <a href="{{ url('') }}/admin/vagas/{{$position->id}}/edit" class="btn btn-sm btn-circle btn-primary">
